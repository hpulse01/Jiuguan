...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!tag) return { title: "标签未找到 - 酒馆" };

  return {
    title: `#${tag.name} - 标签 - 酒馆`,
    description: `浏览标签「${tag.name}」下的失败案例`,
  };
}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-center gap-1 mb-6 bg-stone-900/50 rounded-lg p-1 w-fit border border-stone-800/40">
          {sortTabs.map((tab) => (
            <Link
              key={tab.key}
              href={`/tags/${slug}?sort=${tab.key}&page=1`}
        ...
        {cases.length > 0 ? (
          ...
          <div className="text-center py-20">
            <Hash className="h-12 w-12 text-stone-600 mx-auto mb-4" />
        ...
        {totalPages > 1 && (
          <nav className="flex items-center justify-center gap-2 mt-10">
            {page > 1 && (
              <Link
                href={`/tags/${slug}?sort=${sort}&page=${page - 1}`}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((p, idx) =>
                  p === "ellipsis" ? (
                    ...
                    <Link
                      key={p}
                      href={`/tags/${slug}?sort=${sort}&page=${p}`}
